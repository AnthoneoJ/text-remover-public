...
def get_base_fname(file_path: str) -> str:
    """An OS independent method to extract basename"""
...
def run_draw_mask(coor_input_folder, input_folder, mask_output_folder, radius=0, output_real_image=False):
    ...
    for file in files:
        ...
        file_name_ext = get_base_fname(file)

        file_name = file_name_ext.split(".")[0]
        ext = file_name_ext.split(".")[1]
        gt_file = f"{coor_input_folder}/res_{file_name}.txt"
        ...
        ext = 'png'
        cv2.imwrite(f"{mask_output_folder}/{file_name}_mask.{ext}", img_mask)
        
        if output_real_image:
            cv2.imwrite(f"{mask_output_folder}/{file_name}.{ext}", img)
